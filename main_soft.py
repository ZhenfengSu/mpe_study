...
from mine_lr import create_scheduler
from mine_optim import create_optimizer
...
import models
from models import Learnable_Gelu_hard, Learnable_Atten_hard, Learnable_Block
# import models_v2

import utils
def soft_update(args, slope_param, flag_list_last, rate):

    prune_num = len(slope_param) - int(args.L0_SPARSITY*len(slope_param))
    m = torch.tensor(slope_param).flatten()
    sorted, _ = m.sort()
    
    flat_bm = torch.tensor(flag_list_last).flatten()
    ...
    # flag_bm 转化为 list
    flag_list = flat_bm.tolist()
    # 强制转化为int
    flag_list = [int(i) for i in flag_list]
    return flag_list
...
def swap_attn4model(model, current="", device="cpu"):
    ...
    for name, child in name_to_child.items():
        ...
        if isinstance(child, Attention):
            print(f"Swapping attention for {fqn}")
            new_act = Learnable_Atten_hard(attention=child, device=device)
            setattr(model, name, new_act)
        else:
...
def swap_block4model(model, current="", device="cpu"):
    ...
    for name, child in name_to_child.items():
        ...
        if isinstance(child, Block):
            print(f"Swapping block for {fqn}")
            new_block = Learnable_Block(block=child, device=device)
...
def get_args_parser():
    ...
    parser.add_argument("--L0_SPARSITY", type=float, default=0.7)
    return parser
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and utils.is_main_process():
            with (output_dir / "log.txt").open("a") as f:
                f.write(str(slope_param) + "\n")
        if flag_list_last is None:
            id_list = np.argsort(slope_param)[::-1][:int(args.L0_SPARSITY*len(slope_param))]

            flag_list = np.zeros(len(slope_param))
            flag_list[id_list] = 1
        else:
            rate = math.ceil((1 - epoch / 250)**4)
            flag_list = soft_update(args, slope_param, flag_list_last, rate)

        i = 0
        for module in model.modules():
            if isinstance(module, Learnable_Gelu_hard,) or isinstance(module,Learnable_Atten_hard ) :
